...
def _host_md(h: HostReport, open_details: bool) -> List[str]:
    ...
    bits = [f"{changes} change{'s' if changes != 1 else ''}"]
    if blind:
    ...
    out = [
        ...
        f"<summary><b>{_esc(h.host)}</b> — {_esc(' · '.join(bits))}</summary>",
        "",
    ]
    interesting = (
        Category.CHANGED,
...
def render_markdown(report: RunReport) -> str:
    ...
    for host in sorted(report.hosts):
        out.extend(_host_md(report.hosts[host], open_details=hosts_total <= 3))
